...
const Customers: React.FC = () => {
  ...
  const getCustomers = async (data: {
    ...
    try {
      const response: AxiosResponse<ApiResponse<PaginatedResponse<Customer>>> =
        await api.post("/get-customers", data, {
  ...
  const createCustomer = async (data: {
    ...
    try {
      const response: AxiosResponse<ApiResponse<Customer>> = await api.post(
        "/create-customer",
        data,
        {
          headers: { Authorization: `Bearer ${token}` },
  ...
  const toggleBlock = async (data: {
    ...
    try {
      ...
      throw new Error(
        error.response?.data?.message || "Failed to toggle block"
      );
    }
  };
  ...
  const handleToggleBlock = async (_id: string, currentBlocked: boolean) => {
    ...
    try {
      ...
      Toast.fire({
        icon: "success",
        title: `Customer ${
          !currentBlocked ? "blocked" : "unblocked"
        } successfully!`,
      });
      fetchCustomers();
    } catch (error: any) {
